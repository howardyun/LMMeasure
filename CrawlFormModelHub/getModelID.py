...
def process_models(models, monthly_models, start_date, end_date):
    """处理模型列表，将其分类到每个月份或未知分类。"""
    count = 0
    for model in models:
        created_at = model.created_at  # 直接是 offset-aware datetime 对象
        if created_at:
        ...
        if model.modelId not in monthly_models[month_key]:
            monthly_models[month_key].append(model.modelId)
...
def main(api_token, output_dir, start_date, end_date):
    ...
    models = api.list_models(full=False)
    ...
    print(f"所有模型文件已更新到目录: {output_dir}")
